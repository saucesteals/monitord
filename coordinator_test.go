...
func TestWorkerHandshakeCommitAckAndStop(t *testing.T) {
	...
	go func() { done <- serveWorker[runtimeState](runtimeMonitor{}, desc, workerIn, workerOut) }()
	enc := json.NewEncoder(daemonOut)
	read := bufio.NewReader(daemonIn)
	hello := DaemonFrame{Type: "hello", Hello: &Hello{Version: ProtocolVersion{Major: 5}, DeploymentID: "deployment", DeploymentName: "test", Generation: 2, WorkerToken: "token", ArtifactHash: "artifact", ConfigHash: "config", StateRevision: 7, State: json.RawMessage(`{"count":0}`), Capabilities: []string{"transactions", "outbox", "health"}}}
	...
	if err = enc.Encode(DaemonFrame{Type: "start", Start: &Start{Plan: desc}}); err != nil {
		t.Fatal(err)
	}
	...
	if err = enc.Encode(DaemonFrame{Type: "ack", Ack: &ack}); err != nil {
		t.Fatal(err)
	}
	seenStopped := false
...
func TestWorkerResendsExactTransactionBytes(t *testing.T) {
	...
	go serveWorker[runtimeState](runtimeMonitor{}, desc, workerIn, workerOut)
	enc := json.NewEncoder(daemonOut)
	read := bufio.NewReader(daemonIn)
	_ = enc.Encode(DaemonFrame{Type: "hello", Hello: &Hello{Version: ProtocolVersion{Major: 5}, DeploymentID: "d", Generation: 1, WorkerToken: "t", State: json.RawMessage(`{}`), Capabilities: []string{"transactions", "outbox", "health"}}})
	_, _ = read.ReadBytes('\n')
	_ = enc.Encode(DaemonFrame{Type: "start", Start: &Start{Plan: desc}})
	_, _ = read.ReadBytes('\n')
	...
	_ = enc.Encode(DaemonFrame{Type: "stop", Stop: &Stop{Reason: "shutdown"}})
	...
	_ = enc.Encode(DaemonFrame{Type: "ack", Ack: &TransactionAck{DeploymentID: "d", Generation: 1, Sequence: 1, PayloadHash: out.Transaction.PayloadHash, ResultRevision: 1, Status: "replayed"}})
	for {
...
func TestCommitCancellationAdmissionBoundary(t *testing.T) {
	...
	coord := &workerCoordinator{wire: &wire{w: writer}, hello: Hello{DeploymentID: "d", Generation: 1, WorkerToken: "t"}, acks: make(chan TransactionAck, 1), fatal: make(chan error, 1), progress: make(chan struct{}, 1)}
...
func readOutboundLine(t *testing.T, r *bufio.Reader) WorkerFrame {
	t.Helper()
	raw, err := r.ReadBytes('\n')
	if err != nil {
		t.Fatal(err)
